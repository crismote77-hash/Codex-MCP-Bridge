...
function printHelp() {
  process.stdout.write(`Configure Codex MCP Bridge for Codex + Claude + Gemini.\n\n`);
  ...
  process.stdout.write(`  --server-name <name>   MCP server name (default: codex-bridge)\n`);
  process.stdout.write(`  --command <cmd>        Command (default: codex-mcp-bridge)\n`);
}
...
function upsertCodexConfig(content, { serverName, command, args }) {
  ...
  out = upsertTomlKeyInTable(out, table, "args", `args = ${JSON.stringify(args)}`);
...
function upsertGeminiConfig(obj, { serverName, command, args }) {
  const out = isPlainObject(obj) ? obj : {};
  if (!isPlainObject(out.mcpServers)) out.mcpServers = {};
  out.mcpServers[serverName] = { command, args };
  return out;
...
function configureUser(user, opts) {
  ...
  if (opts.gemini) {
    ...
    const afterObj = upsertGeminiConfig(beforeObj, opts);
    const afterText = JSON.stringify(afterObj, null, 2) + "\n";
    if ((beforeText ?? "") !== afterText) {
