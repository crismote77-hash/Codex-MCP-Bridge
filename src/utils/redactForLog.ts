  ...
  for (const [key, value] of Object.entries(args)) {
    ...
    if (typeof value === "string") {
      ...
      if (key === "prompt" || key === "diff" || key === "content") {
        ...
        // For other string fields, include but redact sensitive patterns
        sanitized[key] = redactSensitiveString(value);
      }
    } else if (Array.isArray(value)) {
      // For arrays, include count and sanitize string elements
      sanitized[`${key}Count`] = value.length;
      if (level !== "errors") {
        sanitized[key] = value.map((item) =>
          typeof item === "string" ? redactSensitiveString(item) : item,
        );
      }
      ...
      // Primitives (numbers, booleans) are safe to include
      sanitized[key] = value;
    }
  }
